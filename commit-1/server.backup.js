...
app.get('/', (req, res) => {
    res.send(`
        ...
        <body>
            ...
            <button onclick="gravarLetra('A'); exibirLetras()">Gravar Conteudo A</button>
            <button onclick="gravarLetra('B'); exibirLetras()">Gravar Conteudo B</button>
            <button onclick="updateRotinas()">Exibir Letras</button>
...
// Rota para gravar a letra no arquivo
app.post('/gravar', express.json(), (req, res) => {
    const { rotinas } = req.body;
    
    // Lê o conteúdo atual do arquivo
    fs.readFile('rotinas.txt', 'utf-8')
    .then(conteudoAtual => {
        // Acrescenta a nova letra ao conteúdo
        const novoConteudo = `${conteudoAtual}${conteudo}\n`;

        // Escreve o novo conteúdo no arquivo
        return fs.writeFile('rotinas.txt', novoConteudo);
    })
    .then(() => {
